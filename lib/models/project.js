...
const findOneWithMethodsAndEvents = whereClause => {
  ...
  const subMethods = pg('method')
     .select(pg.raw('json_agg(json_build_object(\'name\', name, \'type\', type, \'response\', response, \'params\', (select json_agg(json_build_object(\'name\', name, \'description\', description, \'required\', required, \'type\', type) ORDER BY required DESC, name ASC) from "param" inner join "method_param" on "method_param"."param_uuid" = "param"."param_uuid" where "method_uuid" = "method"."method_uuid")) ORDER BY name ASC)'))
     .where('project_uuid',  projectUuidColumnIdentifier)
     .andWhereRaw('name NOT LIKE \'\\_\\_%\'')
     .as('methods');

    const subEvents = pg('event')
       .select(pg.raw('json_agg(json_build_object(\'name\', name, \'params\', (select json_agg(json_build_object(\'name\', name, \'description\', description, \'required\', required, \'type\', type)) from "param" inner join "event_param" on "event_param"."param_uuid" = "param"."param_uuid" where "event_uuid" = "event"."event_uuid")) ORDER BY name ASC)'))
       .where('project_uuid',  projectUuidColumnIdentifier)
       .as('events');
